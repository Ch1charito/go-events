    ...
    async runImport(): Promise<void> {
        ...
        try {
            ...
            if (!Array.isArray(parsed)) {
                throw new Error('JSON muss ein Array von Events sein.');
            }
            const events = parsed.map((e) => ({
                ...e,
                createdAt: e.createdAt ?? new Date().toISOString(),
            }));
            const count = await this.adminService.importEvents(events);
            this.importResult.set(`${count} Events erfolgreich importiert.`);
            this.importJson.set('');
        } catch (err) {
            this.importResult.set(err instanceof Error ? err.message : 'Fehler beim Import.');
        } finally {
